...
def readtable(mysqlurl, username, password, tablename):
    ...
    # 读取表中数据
    sql = 'select  发现时间,结果满意度,案件大类, 街镇,count(*) 数量 from ' + tablename +"   group by 发现时间,结果满意度,案件大类,街镇 "
...
def writetable(mysqlurl, username, password, tablename, dfr):
    ...
    sql = 'insert into ' + tablename + ' (GridNumber,StreetName,Name,Type,theme,subtheme,amount) values (%s,%s,%s,%s,%s,%s,%s)'
