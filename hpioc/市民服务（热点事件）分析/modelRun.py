...
def SQLdata(args):
    ...
    import algorithms.tzsb3 as tb1
    ...
    wtable1 = args['wtable1']
    df = tb.readtable(url, username, password, tablename)

    logger.info("成功读取全量数据")

    df_xy, df_final_ = ex.data_extract(df)



    logger.info("成功提取筛选工单")

    tb.writetable(url1, username, password, wtable, df_xy)
    tb1.writetable(url1, username, password, wtable1, df_final_)
