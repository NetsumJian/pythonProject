...
def readtable(mysqlurl, username, password, tablename):
    ...
    # 读取表中数据
    sql = 'select * from ' + tablename + "   where infotypename like '%%热线%%'   "
...
def writetable(mysqlurl, username, password, tablename, dfr):
    ...
    lst = narray.tolist()
    try:
        sql = 'insert into ' + tablename + ' (id,infozcname,streetname,kwords,description,myd,x,y,amount,fkjl,kword_num,kword_myd) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) '
