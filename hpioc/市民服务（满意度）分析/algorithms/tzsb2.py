...
def readtable(mysqlurl, username, password, tablename):
    ...
    # 读取表中数据
    sql = "select extract(year from discovertime) ,userevaluate,infozcname, streetname, count(*) from " + tablename + "  where infotypename like  '%%热线%%'   group by extract(year from discovertime) ,userevaluate,infozcname, streetname   "
...
def writetable(mysqlurl, username, password, tablename, dfr):
    ...
    try:
        sql = 'insert into ' + tablename + ' (street,hjbh,msbz,shfw,aqgl,whsh,jjfz, xfzs ) values (%s,%s,%s,%s,%s,%s,%s,%s)'
