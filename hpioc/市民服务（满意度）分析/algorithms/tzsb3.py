...
def readtable(mysqlurl, username, password, tablename):
    ...
    # 读取表中数据
    sql = 'select  discovertime,userevaluate,infozcname, streetname,count(*) 数量 from ' + tablename +"  where infotypename like  '%%热线%%'   group by discovertime,userevaluate,infozcname, streetname "
...
def writetable(mysqlurl, username, password, tablename, dfr):
    ...
    sql = 'insert into ' + tablename + ' (datayear,xfzs) values (%s,%s)'
