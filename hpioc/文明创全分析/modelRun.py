...
def SQLdata(args):
    ...
    url = args['url']  # 'jdbc:postgresql://215.0.1.20:25308/epoint_ztk'
    url1 = args['url1']  # 'jdbc:postgresql://215.0.1.20:25308/epoint_ztk'
    username = args['username']   #'epoint'
    password = args['password']   #'Epoint@123'
    tablename = args['tablename']   #'public.mv_taskinfo2'
    tablename_wg = args['tablename_wg']   #'public.mv_taskinfo2'
    wtable=args['wtable']
    #df_ = tb.readtable(url, username, password, tablename)
    logger.info("成功读取全量数据")

    df = ex.data_extract(url,tablename)
