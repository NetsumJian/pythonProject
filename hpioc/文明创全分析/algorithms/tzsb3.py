...
def readtable(mysqlurl, username, password, tablename):
    ...
    # 读取表中数据
    sql = 'SELECT * FROM ' + tablename +"   where (DISCOVERTIME<=(select date_trunc('month',current_date) + interval'1 month - 1 day')  and (DISCOVERTIME>=(select date_trunc('month',current_date)))) "
...
def writetable(mysqlurl, username, password, tablename, dfr):
    ...
    sql = 'insert into ' + tablename + ' (taskid,description,discovertime,theme,subtheme,streetname,communityname,' + \
          'infotypename,address)values(%s,%s,%s,%s,%s,%s,%s,%s,%s)'
